...
(function() {
  ...
  CHEMVENTUR.UI = {
    ...
    bindButtons() {
      ...
      // STAGE BUTTONS! 🎻⚛️🧬
      document.getElementById('btn-stage-0').onclick = () => {
        if (game.stage !== 0) {
          game.changeStage(-game.stage); // Go to stage 0
          this.updateStageButtons();
          this.showStatus('🎻 Stage 0: String Universe!');
        }
      };
      
      document.getElementById('btn-stage-1').onclick = () => {
        const diff = 1 - game.stage;
        if (diff !== 0) {
          game.changeStage(diff);
          this.updateStageButtons();
          this.showStatus('⚛️ Stage 1: Subatomic!');
        }
      };
      
      document.getElementById('btn-stage-2').onclick = () => {
        const diff = 2 - game.stage;
        if (diff !== 0) {
          game.changeStage(diff);
          this.updateStageButtons();
      ...
      // 🎵 v118 MUSIC PLAYER BUTTON 🎵
      const musicBtn = document.getElementById('btn-music-player');
      if (musicBtn) {
        musicBtn.onclick = () => {
          if (CHEMVENTUR.MusicPlayer) {
            if (!CHEMVENTUR.MusicPlayer.enabled) {
              CHEMVENTUR.MusicPlayer.init();
            }
            CHEMVENTUR.MusicPlayer.toggleUI();
          }
        };
      }
